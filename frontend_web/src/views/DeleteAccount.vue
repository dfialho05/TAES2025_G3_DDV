    ...
    >
      ...
      >
        <p class="text-sm text-red-800 dark:text-red-200">
          <strong>Aviso:</strong> Ao apagar a sua conta, todos os seus dados, incluindo histórico de
          jogos, estatísticas e moedas serão permanentemente removidos. Esta ação não pode ser
          desfeita.
        </p>
      </div>
      ...
      <div class="space-y-4 mb-6">
        <div>
          <label class="block text-sm font-medium mb-2">Confirme o seu email</label>
          <input
        ...
        <div class="flex items-start gap-2">
          <input v-model="confirmed" type="checkbox" id="confirm" class="mt-1" />
          <label for="confirm" class="text-sm text-gray-600 dark:text-gray-400">
            Compreendo que esta ação é permanente e não pode ser desfeita
          </label>
        </div>
      </div>
...
const canDelete = computed(() => {
  return confirmEmail.value === authStore.user.email && password.value.length > 0 && confirmed.value
})
