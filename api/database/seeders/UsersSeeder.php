...
class UsersSeeder extends Seeder
{
    ...
    private function addUsersToDatabase()
    {
        ...
        $createdDate = DatabaseSeeder::$startDate
            ->copy()
            ->addMinutes(mt_rand(20000, 100000));
        foreach ($usersAdded as $key => $user) {
            ...
            $usersAdded[$key]["email_verified_at"] = $createdDate
                ->copy()
                ->addMinutes(mt_rand(1, 9));
            if (random_int(1, 7) > 1) {
                $usersAdded[$key]["updated_at"] = $createdDate
                    ->copy()
                    ->addMinutes(mt_rand(10, 10000));
            } else {
                $usersAdded[$key]["updated_at"] = $createdDate;
            }
            $usersAdded[$key]["nickname"] =
                ($user["gender"] == "M" ? "Mickey" : "Minnie") . ($key + 1);
            ...
            if ($usersAdded[$key]["softdelete"]) {
                $usersAdded[$key]["deleted_at"] = $createdDate
                    ->copy()
                    ->addMinutes(mt_rand(10001, 100000));
            }
        }
        ...
        foreach ($usersAdded as $user) {
            ...
            if (count($arrayToStore) >= DatabaseSeeder::$dbInsertBlockSize) {
                ...
                $this->command->line(
                    "Created " . count($arrayToStore) . " users",
                );
                $arrayToStore = [];
            }
        }
        ...
        $this->command->line(
            "Total users created: " . DB::table("users")->count(),
        );
    ...
    private function addPhotoFiles()
    {
        ...
        $sortedUsers = self::$dbUsers->sortBy(function (stdClass $user) {
            ...
            return match ($user->type) {
                ...
                default => $user->id < 50
                    ? 2000 + $user->id
                    : 3000 + mt_rand(0, 100000),
            };
        });
        $i = 0;
        foreach ($sortedUsers as $user) {
            $originalFilename = str_starts_with($user->nickname, "Mickey")
            ...
            if ($originalFilename) {
                ...
                $newFileName = $this->copyFileToStorage(
                    "photos",
                    $originalFilename,
                    "photos_avatars",
                    $user->id,
        ...
        $this->directCopyFileToStorage(
            "photos",
            "anonymous.png",
            "photos_avatars",
        );
        $this->command->line(
            "Image for user with no associated photo was copied!",
        );
    }
    ...
    private function addBotUser()
    {
        $this->command->line("Adding BOT user");
