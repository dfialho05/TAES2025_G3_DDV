...
// Starts a multiplayer game when host initiates it
export const startMultiplayerGame = (manager, gameId, hostName, io) => {
  try {
    const game = manager.activeGames.get(gameId);
    if (!game) {
      return { success: false, error: "Sala não encontrada" };
    ...
    if (room.host !== hostName) {
      return { success: false, error: "Apenas o host pode iniciar o jogo" };
    }

    if (room.players.length < 2) {
      return { success: false, error: "Pelo menos 2 jogadores necessários" };
    }
    ...
    io.to(gameId).emit("multiplayerGameStarted", {
      gameId,
      state: gameState,
...
// Returns a list of available public rooms that can be joined
export const getAvailableRooms = (manager) => {
  const rooms = [];

  // Filter only public, non-started multiplayer rooms
  for (const [gameId, game] of manager.activeGames) {
    if (
      game.roomInfo &&
      !game.roomInfo.isPrivate &&
      !game.roomInfo.gameStarted
    ) {
      rooms.push({
        gameId,
        ...
        needsPassword: !!game.roomInfo.password,
        createdAt: game.roomInfo.createdAt,
      });
    }
  }
...
// Updates room settings (only host can modify)
export const updateRoomSettings = (game, hostName, newSettings, io, gameId) => {
  try {
    if (!game.roomInfo) {
      return { success: false, error: "Não é uma sala multiplayer" };
    }

    if (game.roomInfo.host !== hostName) {
      return {
        success: false,
    ...
    const { roomName, isPrivate, password, turnTime } = newSettings;

    // Update settings if provided and valid
    if (roomName) game.roomInfo.roomName = roomName;
    if (typeof isPrivate === "boolean") game.roomInfo.isPrivate = isPrivate;
    if (password !== undefined) game.roomInfo.password = password;
    if (turnTime && turnTime >= 5 && turnTime <= 300) {
      game.roomInfo.turnTime = turnTime;
      game.turnTime = turnTime * 1000;
    }
