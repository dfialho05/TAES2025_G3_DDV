import * as card from "./CardClass.js";
...
const selectPlayWinner = (card1, card2, trump) => {
  ...
  if (
    !(card1 instanceof card.Card) ||
    !(card2 instanceof card.Card) ||
    !!suits.hasOwnProperty(trump)
  ) {
    throw new Error(
  ...
  // If they are of different suits and neither is trump, the first card wins by default
  return { winner: card1.getSuit() === trump ? 1 : 2, card1, card2 };
};

const verifyDeckEmpty = (deck) => {
  // Verify if deck is empty
  return deck.length === 0 ? 1 : 0;
};

const doAPlay = (deck, trump, player1Card, player2Hand, player2Card) => {
  // Verify if deck is empty and both cards has the same suit
  if (
    verifyDeckEmpty(deck) &&
    player1Card.getSuit() !== player2Card.getSuit()
  ) {
