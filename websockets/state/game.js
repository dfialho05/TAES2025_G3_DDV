  ...
  const dbCallbacks = {
    onGameStart: async () => {
      ...
      if (mode === 'multiplayer') {
          if (!newGame || !newGame.player2) {
              console.log(`⏳ [State] Multiplayer: Jogo em memória. À espera de P2 para registar na BD.`);
              return null;
          }
      }

      let gId = null;
      const p2Id = (mode === 'multiplayer' && newGame.player2) ? newGame.player2.id : null;
      ...
      if (isMatch && dbMatchId) {
        ...
        console.log(` 🔸 [DB] Game Solto criado: ID ${gId} (P1: ${user.id}, P2: ${p2Id || 'Bot'})`);
      }
      return gId;
    },
    ...
    onGameEnd: async (gameDbId, winnerSide, p1Points, p2Points) => {
      ...
      if (winnerSide === "player1") realWinnerId = user.id;
          ...
          realWinnerId = (mode === 'multiplayer' && newGame.player2) ? newGame.player2.id : BOT_ID;
      }
...
export const joinGame = async (id, user) => {
  ...
  if (game && !game.player2) {
    ...
    if (p1Id === userId) {
        console.warn(`⚠️ [State] Jogador ${user.name} tentou entrar no próprio jogo.`);
        return null;
    }
...
export const handlePlayerMove = (gameID, cardIndex, socketID) => {
  ...
  else if (game.player2 && String(game.player2.id) === userIdStr) side = "player2";
