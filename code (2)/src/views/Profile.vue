<template>
  <div class="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 border border-gray-200 dark:border-gray-700">
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
              ...
              <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ authStore.user.stats.wins }}</p>
            </div>
            
            <div class="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg">
              <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Derrotas</p>
              <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ authStore.user.stats.losses }}</p>
            </div>
            
            <div class="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
              <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Moedas</p>
              <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400">{{ authStore.user.coins }}</p>
            </div>
            
            <div class="p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
              <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Capotes</p>
              <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ authStore.user.stats.capotes }}</p>
            </div>
            
            <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              <p class="text-sm text-gray-600 dark:text-gray-400 mb-1">Bandeiras</p>
              <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ authStore.user.stats.bandeiras }}</p>
            </div>
          </div>
    ...
    <!-- Edit Modal (Mock) -->
    <div v-if="showEditModal" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" @click="showEditModal = false">
